#include<iostream>
 ...
 int minCameraCover(TreeNode* root) {
    if(root == NULL)
        return 0;
    int f1 = 1 + (root->left !=NULL ? minCameraCover(root->left->left) : 0) +
            (root->left !=NULL ? minCameraCover(root->left->right) : 0) +
            (root->right !=NULL ? minCameraCover(root->right->left) : 0) +
            (root->right !=NULL ? minCameraCover(root->right->right) : 0) ;

    int f2 = 1 +  minCameraCover(root->left);

    int f3 = 1 + minCameraCover(root->right);
